...
// fetches a file synchronously. Required due to bespoke plugins limitations
// (they can't finish asynchronously)
...
const slidify = (deck, slide, md) => {
  ...
  switch (true) {
    ...
    case markdownAttribute && markdownAttribute.trim() !== '':
      // we need to fetch synchronously here as due to bespoke plugins 
      // not being able to finish asynchronously, there might be race conditions
      fetchFile(
        markdownAttribute.trim(),
        fileContents => {
          const slidesContent = fileContents.split(/\r?\n---+\r?\n/);
          slidesContent.forEach(function(slideContent) {
            const slideContainer = createSlide(deck, slide);
...
export default (mdConfigOrFactory, metadataCallbacks, pluginsArray) => {
  ...
  return deck => {
    ...
    if (Object.keys(metadataCallbacks).length > 0 && Object.keys(slideMetadata).length > 0) {
